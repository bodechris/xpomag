...
function ObjectPositionGrid({value,onChange}:{value:string;onChange:(v:string)=>void}){const pts: Array<[string,string]>=[["0% 0%","↖"],["50% 0%","↑"],["100% 0%","↗"],["0% 50%","←"],["50% 50%","•"],["100% 50%","→"],["0% 100%","↙"],["50% 100%","↓"],["100% 100%","↘"]];return <div className="xp-focal-grid">{pts.map(([v,l])=><button key={v} className={value===v?"is-active":""} onClick={()=>onChange(v)} title={v}>{l}</button>)}</div>}
